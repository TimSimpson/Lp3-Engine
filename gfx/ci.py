...
def deps(args):
    ...
    print('Checking out Lp3-Core...')
    lp3_core_dir = os.path.join(DEPS_DIR, 'lp3-core')
    mkdir(lp3_core_dir)
    github_check_call([
        'git', 'clone',
        'https://github.com/TimSimpson/Lp3-Core.git',
        lp3_core_dir
    ])
    subprocess.check_call([
        'git', 'checkout',
        '0ba06483af259c128a3fe3677c7930c0f10d0ad2'
    ], cwd=lp3_core_dir)
    
    
    print('Checking out Lp3-Sims...')
    lp3_sims_dir = os.path.join(DEPS_DIR, 'lp3-sims')
    mkdir(lp3_sims_dir)
    github_check_call([
        'git', 'clone',
        'https://github.com/TimSimpson/Lp3-Sims.git',
        lp3_sims_dir
    ])
    subprocess.check_call([
        'git', 'checkout',
        'ecc7a10637d5c2d30a0a26004b883da1d205d927'
    ], cwd=lp3_sims_dir)
    
    
    print('Checking out Lp3-Gl...')
    lp3_gl_dir = os.path.join(DEPS_DIR, 'lp3-gl')
    mkdir(lp3_gl_dir)
    github_check_call([
        'git', 'clone',
        'https://github.com/TimSimpson/Lp3-Gl.git',
        lp3_gl_dir
    ])
    subprocess.check_call([
        'git', 'checkout',
        '0b8446088ace1e66ac46cf6b1fd6981fa0bf7237'
    ], cwd=lp3_gl_dir)
